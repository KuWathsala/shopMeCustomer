...
export default class App extends React.Component{
    constructor(props){
        ...
        this.state={
            ...
            connection: new signalR.HubConnectionBuilder().withUrl("http://192.168.43.16:5001/connectionHub").build()
        }
    }
    ...
    confirm=()=>{
        ...
        axios.get(`http://192.168.43.16:5001/api/orders/updateOrderStatus/${this.state.id},${this.state.orderStatus}`)//Orders/${4},${this.state.status}
        .then(response=>{
        ...
        axios.get(`http://192.168.43.16:5001/api/deliverers/updateDeliveryStatus/${this.state.id},${this.state.delivererStatus}`)//Orders/${4},${this.state.status}
        .then(response=>{
            console.log(response.data);
        })
        .catch(error=>{console.log(error)})
        ...
        this.state.connection.invoke("Reply",  1,"Accepted");
    }

    reject=()=>{
        this.state.connection.invoke("Reply",  1,"Reject");
    }
